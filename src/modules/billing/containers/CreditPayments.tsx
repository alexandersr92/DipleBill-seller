...
export default function CreditPayments() {
  ...
  const sellerId = useAppSelector((state) => state.userSlice.sellerId) || localStorage.getItem('seller_id') || '';
  const sellerName = useAppSelector((state) => state.userSlice.sellerName) || localStorage.getItem('seller_name') || '';
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.get(`/v1/credits/search-active?search=${encodeURIComponent(searchQuery)}`);
                ...
                {credits.map((credit) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center border-t pt-2.5 border-slate-200 dark:border-slate-800">
                      <div>
                        <p className="text-[9px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Total Inicial</p>
                        <p className="text-xs font-black text-slate-700 dark:text-slate-300">{currencyFormatter({ currency: 'NIO', value: credit.total })}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider text-right">Saldo Pendiente</p>
                        <p className="text-sm font-black text-blue-600 dark:text-blue-400">{currencyFormatter({ currency: 'NIO', value: credit.debt })}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="border-2 border-slate-350 dark:border-slate-800 rounded-lg p-4 bg-slate-50 dark:bg-slate-900/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="text-base font-extrabold text-slate-900 dark:text-white">{selectedCredit.client_name}</h2>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Saldo Pendiente</p>
                <p className="text-xl font-black text-blue-600 dark:text-blue-400">{currencyFormatter({ currency: 'NIO', value: selectedCredit.debt })}</p>
              </div>
            </div>
              ...
              {(['CASH', 'TRANSFER', 'CARD'] as const).map((method) => {
                ...
                return (
                  ...
                  >
                    {icon}
                    <span>{label}</span>
                    <kbd className="hidden md:inline-block px-1 rounded bg-white/20 text-[9px] font-semibold">{kbd}</kbd>
                  </button>
                );
              })}
            </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="cashNio" className="text-[11px] font-bold">Córdobas (C$)</Label>
                    ...
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="cashUsd" className="text-[11px] font-bold">Dólares ($)</Label>
                      <div className="relative">
                        <span className="absolute left-2.5 top-1.5 text-xs font-black text-slate-500">$</span>
                        <Input
                          id="cashUsd"
                          type="number"
                          step="any"
              ...
              {paymentMethod === 'TRANSFER' && (
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-1">
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="transBank" className="text-[11px] font-bold">Banco</Label>
                      <select
                        ref={transBankRef}
                        id="transBank"
                    ...
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="transRef" className="text-[11px] font-bold">Referencia</Label>
                      <Input
                        id="transRef"
                        type="text"
                  ...
                  <div className="flex flex-col gap-1">
                    <Label htmlFor="transAmt" className="text-[11px] font-bold">Monto Transferencia (C$)</Label>
                    <div className="relative">
                      <span className="absolute left-2.5 top-1.5 text-xs font-black text-slate-500">C$</span>
                      <Input
                        ref={transAmtRef}
              ...
              {paymentMethod === 'CARD' && (
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-1">
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="cardBrand" className="text-[11px] font-bold">Franquicia</Label>
                      <select
                        ref={cardBrandRef}
                        id="cardBrand"
                    ...
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="cardDigits" className="text-[11px] font-bold">Últimos 4</Label>
                      <Input
                        id="cardDigits"
                        type="text"
                        maxLength={4}
                    ...
                    <div className="flex flex-col gap-1">
                      <Label htmlFor="cardRef" className="text-[11px] font-bold">Voucher</Label>
                      <Input
                        id="cardRef"
                        type="text"
                  ...
                  <div className="flex flex-col gap-1">
                    <Label htmlFor="cardAmt" className="text-[11px] font-bold">Monto Tarjeta (C$)</Label>
                    <div className="relative">
                      <span className="absolute left-2.5 top-1.5 text-xs font-black text-slate-500">C$</span>
                      <Input
                        ref={cardAmtRef}
            ...
            <div className="border-2 border-slate-350 dark:border-slate-800 rounded-lg p-4 bg-slate-50 dark:bg-slate-950/20 flex flex-col gap-4">
              ...
              {isOverpaid && (
                <div className="p-3 bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-400 border border-red-300 dark:border-red-900 rounded-md flex gap-2 items-start text-xs font-bold animate-bounce">
                  ...
                  <span>El monto excede el saldo pendiente (C$ {selectedCredit.debt.toFixed(2)})</span>
                </div>
              )}
              ...
              <div className="flex flex-col gap-2.5 pt-1.5">
                <div className="flex justify-between items-center text-xs font-bold">
                  ...
                  <span className="text-base font-black text-blue-600 dark:text-blue-400">{currencyFormatter({ currency: 'NIO', value: totalPaidInNio })}</span>
                </div>
                <div className="flex justify-between items-center pt-2.5 border-t border-slate-200 dark:border-slate-800">
                  <span className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-200">Deuda Restante:</span>
                  <span className="text-sm font-black text-slate-900 dark:text-white">
                    {currencyFormatter({ currency: 'NIO', value: Math.max(selectedCredit.debt - totalPaidInNio, 0) })}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5 pt-2">
                <Label htmlFor="notes" className="text-xs font-bold">Notas / Detalles</Label>
                <textarea
                  id="notes"
                  value={notes}
